            ...
            public void handle(ActionEvent e)
            {
                if (checkBox.isSelected()) {
                    ...
                    try {
                        /**
                         * LINE READER
                         * Reads through the lines in the csv so once a specific image is selected, if..
                         * previous bounds have been saved, we can call and view those previous bounds on the image
                         */
                        BufferedReader reader = new BufferedReader(new FileReader(excelFile.getAbsolutePath()));
                        List<LabelData> lines = new ArrayList<>();
                        String line = null;
                        while ((line = reader.readLine()) != null) {
                            String[] parts = line.split(","); // each item in this array will be a "column"

                            if(parts[0].trim().equals("Id")) {
                                System.out.println("SKIPPING HEADER");
                                continue;
                            } else {
                                System.out.println("CREATE LABEL DATA FOR " + parts[0]);
                            }
                            ...
                            String rootPath = excelFile.getParent();
                            String imagePath = Paths.get(rootPath.toString(), "images", parts[INDEX_OF_FILE_NAME_COLUMN_IN_CSV]);
                            System.out.println(imagePath);
                            Image image = new Image(imagePath);

                            ld.setImage(image);

                            ld.setImage(new Image(imagePath));
                            ld.setImageID(Integer.parseInt(parts[0]));
                            ld.setSelectedX1(Double.parseDouble(parts[1])); //etc
                            ld.setSelectedY1(Double.parseDouble(parts[2]));
                            ld.setSelectedX2(Double.parseDouble(parts[3]));
                            ld.setSelectedY2(Double.parseDouble(parts[4]));
                            ld.setSelectedX3(Double.parseDouble(parts[5]));
                            ld.setSelectedY3(Double.parseDouble(parts[6]));
                            ld.setSelectedX4(Double.parseDouble(parts[7]));
                            ld.setSelectedY4(Double.parseDouble(parts[8]));

                            lines.add(ld);

                            System.out.println("Added " +ld.getImageID());
                        }

                        int whatToFind = imageID;
                        int foundIndex = -1;

                        for(int i = 0; i < lines.size(); i++) {

                            if(lines.get(i).getImageID() == whatToFind) {
                                System.out.println("FOUND AT" + i);
                                foundIndex = i;
                                break;
                            }
                            else {
                                System.out.println("NOT FOUND AT" + i + " " + lines.get(i).getImageID() + " NOT EQUAL " + whatToFind);
                            }
                        }

                        if(foundIndex < 0) {
                            System.out.println("error finding line #");
                        } else {
                            System.out.println("FOUND: " + lines.get(foundIndex).getImageID());
                        }
