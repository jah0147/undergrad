...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        EventHandler<ActionEvent> checkEvent = new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent e)
            {
                if (checkBox.isSelected()) {
                    ...
                    System.out.println(rubberBandSelection.getBounds());
         ...
         * EXCEL FILE BUTTON
         */
        Button excelBtn = (Button)scene.lookup("#excelFileButton"); //calls the browse button from the fxml file

        excelBtn.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                excelFileChooser(primaryStage);
        ...
        saveBtn.setOnAction((event) -> { //Creates an action for the save button
            try {
                ...
                if (imagesSource != null) { //if statement to check if there is a picture to save
                    ...
                    String [] Title = "Image Number,Min X,Min Y,Max X,Max Y,Width,Height,\n".split(","); //Title for csv file
                    //String [] getBounds = String.valueOf(rubberBandSelection.getBounds()).split(","); //placeholder for testing writing bounds to csv file
                    String [] getBounds = (java.util.Arrays.asList(imageViews).indexOf(imagesSource) + 1 //(the imageNumber Code so we can reference later when
                            + "," + selectionBounds.getMinX() + "," + selectionBounds.getMinY() + "," + selectionBounds.getMaxX()
                            + "," + selectionBounds.getMaxY() + "," + selectionBounds.getWidth()
                            + "," + selectionBounds.getHeight() + "," + "\n").split(","); //String for excel file

                    writer.writeNext(Title);
                    writer.writeNext(getBounds);
                    writer.close();

                    System.out.println("Wrote Title for bounds and the bounds are" + getBounds);
                }
                /**
                 * SAVE POPUP
...
//      }
    ...
    private void excelFileChooser(Stage parent) {
        FileChooser fileChooser = new FileChooser();
        File selectedFile =
                fileChooser.showOpenDialog(parent);

        if (selectedFile != null) {
            FilenameFilter filterExcel = new FilenameFilter() {
                @Override
                public boolean accept(File dir, String name) {
