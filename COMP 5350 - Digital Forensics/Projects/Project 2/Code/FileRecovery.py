# This is a python script that will recover files from a disk in .dd format
#This script was created for COMP 5350 Digital Forensics
#Authors: Jacob Howard, Ally Bennett, & Megan Crews
...
# Setting Up types (File Types supported)
...
# Create new directory for recovered files if not already created to store recovered files
...
for k in foundFile:  
	...
	ff3 = foundFile[k][3]
        ...
        # " if={} ".format(diskFile) allows for any .dd file name.
        # If this is not working, please change line 171 to:
            #cmd = "dd if=Project2.dd of=%s bs=1 skip=%s count=%s status=none"%(ff0, ff1, ff3)
	cmd = "dd if={} of=%s bs=1 skip=%s count=%s status=none".format(diskFile)%(ff0, ff1, ff3)
	os.system(cmd)
...
for k in Output:
	print(Output[k])
#Print message that files recovered were stored in directory
print("\nFiles stored in %s/RecoveredFiles"%pathlib.Path().absolute())

# Makes Text File for outputs
# Text file is stored in the same directory the script is ran from
sys.stdout = open("consolOutput.txt", "w") #make text file to write to (all print statements will be included in .txt)
print("The disk image contains %i files\n"%len(Output))
print("FileName\tFileStart\tFileEnd\t\tFileSize\tSHA256")
for k in Output:
	print(Output[k])
#Print message that files recovered were stored in directory
print("\nFiles stored in %s/RecoveredFiles"%pathlib.Path().absolute())
sys.stdout.close() # Close text file containing consol output
